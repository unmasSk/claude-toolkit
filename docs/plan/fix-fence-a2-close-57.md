...
## Goal
Cerrar la clase de forja/falsificación de la salida de memoria hacia el LLM: muro infalsificable (nonce) en los 2 sitios que hoy lo enmarcan, transporte de bytes correcto en los 2 subprocess, tope anti-ReDoS y cierre de LOW-17 — con verify limpio y Yoda 110.

## Alcance (de `79fdf9a`)
DENTRO:
- **A2 nonce** en `hooks/user-prompt-memory-check.py:266-277` (único muro en producción) y en el delimitador hermano `=== SNAPSHOT ===` de `hooks/precompact-snapshot.py`.
...
- Muro estructural nuevo para `boot-log-latest.txt` / `lib/boot_render.py` (hoy no tiene ninguno; se apoya en saneo por campo). Rediseño separado.

## Cobertura de tests que YA existe (no duplicar)
`tests/test_control_byte_injection.py` (154 tests). Cubierto: forja/borrado por byte de control a nivel registro/campo, evasión de muro `\x1c/\x1d/\x1e/\x85/\x1f` (como bytes→espacio y como `\s`), bypass de strip de tags genéricos, unicidad del delimitador de precompact.
...
- (d) Infalsificabilidad del nonce A2 (un trailer hostil no puede reproducir el muro).
...
- [ ] RED (c): `</memory-data` con `\x1c` antes del `>`; aserción: el muro se neutraliza igualmente (no sobrevive sin cerrar).
- [ ] RED (d): trailer que intenta reproducir literal el marcador de muro; aserción: con nonce por invocación, el marcador inyectado NO coincide con el muro real → no rompe el marco.
...
- [ ] A2 nonce: generar nonce por invocación e insertarlo en el muro de `user-prompt-memory-check.py`; mismo patrón en el delimitador de `precompact-snapshot.py`.
...
- Argus (re-verifica SEC-CRIT-16, LOW-17, clase de muro)
