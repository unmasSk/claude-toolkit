...
| **Capa 6** | 2 hooks *(`inject.py` retirado entero — decisión del propietario, 2026-08-03, B20)* | ✅ construidos y con test **[comprobado 2026-08-03, tras B20: `customs.py` · `boot_launcher.py` existen en `unmassk-toolkit/hooks/`, `pytest unmassk-toolkit/tests/memory/test_customs_hook.py unmassk-toolkit/tests/memory/test_boot_launcher.py -q` → 29 passed]**. ~~Sin enchufar en `hooks.json` a propósito: engancharlos mientras el sistema viejo sigue vivo dispararía dos arranques a la vez. Falta apuntarlos — `DEUDA.md` punto 26~~ **Enchufados 2026-08-05, al publicar versión:** `unmassk-toolkit/hooks/hooks.json` registra `boot_launcher.py` en `SessionStart` y `customs.py` en `PreToolUse` (`matcher: "Bash"`), verificado leyendo el fichero. `DEUDA.md` punto 26, cerrado |
