...
class IndexOptimizer:
    ...
    def _create_single_column_recommendation(
        ...
        return IndexRecommendation(
            ...
            sql_statement=f"CREATE INDEX {_validate_identifier(index_name)} ON {_validate_identifier(table_name)} ({_validate_identifier(column)});",
    ...
    def _create_composite_recommendation(
        ...
        return IndexRecommendation(
            ...
            sql_statement=f"CREATE INDEX {_validate_identifier(index_name)} ON {_validate_identifier(table_name)} ({', '.join(_validate_identifier(c) for c in ordered_columns)});",
            selectivity_analysis={
    ...
    def _create_covering_index_recommendation(
        ...
        safe_idx = _validate_identifier(index_name)
        safe_tbl = _validate_identifier(table_name)
        safe_idx_cols = ', '.join(_validate_identifier(c) for c in index_columns)
        sql_parts = [f"CREATE INDEX {safe_idx} ON {safe_tbl} ({safe_idx_cols})"]
        if include_columns:
            safe_inc_cols = ', '.join(_validate_identifier(c) for c in include_columns)
            sql_parts.append(f" INCLUDE ({safe_inc_cols})")
        ...
        for index in indexes:
            ...
            if signature in seen_signatures:
                ...
                issues.append(RedundancyIssue(
                    ...
                    sql_statements=[f"DROP INDEX {_validate_identifier(index.name)};"]
                ))
            else:
            ...
            for index2 in indexes[i+1:]:
                ...
                if overlap_ratio >= self.redundancy_overlap_threshold:
                    ...
                    issues.append(RedundancyIssue(
                        ...
                        sql_statements=[f"DROP INDEX {_validate_identifier(redundant_index.name)};"]
                    ))
