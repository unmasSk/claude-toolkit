...
class SchemaAnalyzer:
    def __init__(self):
        ...
        # Naming conventions
        self.table_naming_pattern = re.compile(r'^[a-z][a-z0-9_]*$')
        self.column_naming_pattern = re.compile(r'^[a-z][a-z0-9_]*$')
            ...
            for column in table.columns:
                if column.foreign_key and column.name not in existing_indexed_columns:
                    missing_indexes.append({
                        ...
                        'column': column.name,
                        'type': 'foreign_key',
                        'suggestion': f"CREATE INDEX {_validate_identifier(f'idx_{table_name}_{column.name}')} ON {_validate_identifier(table_name)} ({_validate_identifier(column.name)});"
                    })
