...
```sql
SELECT pid, state, now() - xact_start AS tx_age FROM pg_stat_activity WHERE xact_start IS NOT NULL ORDER BY xact_start;
```
...
- Tune autovacuum per-table for write-heavy tables; do not change global defaults first.
