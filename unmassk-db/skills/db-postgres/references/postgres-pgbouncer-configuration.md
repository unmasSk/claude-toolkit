...
Multiplication: 2 users x 3 databases = 6 x default_pool_size connections.
Example: 45 with 2 users and 3 databases = 270 backend connections.

Recommended values:
- 1 or few database/user pairs (OLTP): 25-50
- High number of database/user pairs active simultaneously: 10-25

## max_user_connections

Max backend connections per user across all databases. Set in the `[users]` section. Default: 0 (unlimited).

Recommended: `0.7-0.85 x Postgres max_connections` to leave headroom for direct access.
