...
`work_mem` is per-operation, not per-query. Estimate: `work_mem x operations_per_query x parallel_workers x connections` can grow very large at high concurrency. Scale connections and parallelism before raising `work_mem`.

## Connection Pooling

Each connection = OS process (fork overhead, context switching, memory). PgBouncer can multiplex many application connections to fewer database connections. Typical: 1000 application connections through pooler to 20-50 backends. Implement pooling before raising `max_connections`; `max_connections` requires a full restart to change (default 100). Note: `superuser_reserved_connections` (default 3) reserves slots for emergency superuser access, so non-superusers are rejected before `max_connections` is fully reached.
...
| Stuck process | `SELECT pg_cancel_backend(pid);` then `SELECT pg_terminate_backend(pid);` -- confirm with a human before terminating backends, as this may abort in-flight transactions |
