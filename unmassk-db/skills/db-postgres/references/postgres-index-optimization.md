...
Indexes with identical definitions on the same table are duplicates:
...
Warning: Confirm with a human before dropping duplicate indexes. Some apparent "duplicates" differ in operator classes, collations, predicates, or sort order and may be required for critical workloads.

## Identify Invalid Indexes

Failed `CREATE INDEX CONCURRENTLY` builds leave INVALID indexes that are maintained on every write but never used for reads. `CREATE INDEX CONCURRENTLY IF NOT EXISTS` silently succeeds if an invalid index already exists -- always check and drop before retrying.
...
Key levers: set `fillfactor = 70-80` on write-heavy tables; never index frequently-updated columns (`status`, `updated_at`) unless query-critical; use partial indexes to reduce scope. PG 16+: BRIN indexes are excluded from HOT eligibility checks.

## Write Amplification

Each additional index adds write-path overhead because every INSERT/UPDATE/DELETE must maintain more index entries. Moving from 7 to 39 indexes can cause a ~58% throughput drop on write-heavy workloads.

To reduce WAL volume from extra write activity, enable `wal_compression` (`lz4` and `zstd` options are PG 15+). Tune `max_wal_size` separately to reduce checkpoint frequency under sustained write load.
