...
- `checkpoint_completion_target=0.9` spreads I/O over 90% of the interval; avoids spikes.
- "Checkpoints are occurring too frequently" in logs means increase `max_wal_size`.
...
Backend-direct writes (formerly `buffers_backend` in `pg_stat_bgwriter`) are now tracked in `pg_stat_io`:

```sql
SELECT writes FROM pg_stat_io WHERE backend_type = 'client backend' AND object = 'relation';
```

## Crash Recovery

On crash, PostgreSQL replays WAL from the last checkpoint. Longer checkpoint intervals mean more WAL to replay and longer recovery. For most workloads, `checkpoint_timeout=5min` and `max_wal_size` tuned to keep checkpoints time-based is the right balance.
