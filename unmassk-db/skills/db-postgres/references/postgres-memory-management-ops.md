...
- **Shared memory**: `shared_buffers` -- main data cache, all processes share it, requires restart to change.
...
- **Hash multiplier**: `hash_mem_multiplier` (default 2.0) means hash operations use up to 2x `work_mem`.

## Memory Multiplication Danger

Maximum potential: `work_mem x operations_per_query x (parallel_workers + 1) x connections` (leader participates by default via `parallel_leader_participation = on`; hash operations use up to `hash_mem_multiplier x work_mem`, default 2.0).

Example: 128MB work_mem, 3 ops (2 sorts + 1 hash join), 2 parallel workers, 100 connections:
- 2 sorts at 128MB = 256MB
- 1 hash join at 128MB x 2.0 = 256MB
- per process = 512MB
- x 3 processes (2 workers + leader) = 1536MB/query
- x 100 connections = ~150GB worst case

Not all queries hit limits simultaneously, but high concurrency + large datasets approach it. This is a common cause of OOM in containerized and Kubernetes deployments. Plan capacity with a 1.5-2x safety margin.
...
- `maintenance_work_mem` can be set much higher (1-2GB) -- fewer processes use it. Cap autovacuum with `autovacuum_work_mem` to avoid `autovacuum_max_workers x maintenance_work_mem` memory spikes.
