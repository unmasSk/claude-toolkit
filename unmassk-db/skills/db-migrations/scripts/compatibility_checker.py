...
class SchemaCompatibilityChecker:
    ...
    def analyze_database_schema(self, before_schema: Dict[str, Any], 
        ...
        for table_name in after_tables:
            if table_name not in before_tables:
                safe_new_table = _validate_identifier(table_name)
                migration_scripts.append(MigrationScript(
                    ...
                    rollback_script=f"DROP TABLE IF EXISTS {safe_new_table};",
                    dependencies=[],
        ...
        for col_name in after_columns:
            if col_name not in before_columns:
                ...
                scripts.append(MigrationScript(
                    ...
                    script_content=f"ALTER TABLE {safe_table} ADD COLUMN {self._generate_column_definition(col_name, col_def)};",
                    rollback_script=f"ALTER TABLE {safe_table} DROP COLUMN {safe_col};",
                    dependencies=[],
    ...
    def _analyze_column_changes(self, table_name: str, col_name: str, 
        ...
        if before_type != after_type:
            ...
            if compatibility == "breaking":
                ...
                scripts.append(MigrationScript(
                    ...
                    script_content=f"ALTER TABLE {safe_table} ALTER COLUMN {safe_col} TYPE {after_type} USING {safe_col}::{after_type};",
                    rollback_script=f"ALTER TABLE {safe_table} ALTER COLUMN {safe_col} TYPE {before_type};",
                    dependencies=[f"backup_{table_name}"],
                    validation_query=f"SELECT COUNT(*) FROM {safe_table} WHERE {safe_col} IS NOT NULL;"
                ))
        ...
        if before_nullable != after_nullable:
            if before_nullable and not after_nullable:  # null -> not null
                ...
                safe_table2 = _validate_identifier(table_name)
                safe_col2 = _validate_identifier(col_name)
                scripts.append(MigrationScript(
                    script_type="sql",
                    description=f"Make column {col_name} NOT NULL",
                    script_content=f"""
                    -- TODO: Replace 'REPLACE_WITH_ACTUAL_DEFAULT' with the real default value before executing
                    UPDATE {safe_table2} SET {safe_col2} = 'REPLACE_WITH_ACTUAL_DEFAULT' WHERE {safe_col2} IS NULL;
                    -- Add NOT NULL constraint
                    ALTER TABLE {safe_table2} ALTER COLUMN {safe_col2} SET NOT NULL;
                    """,
                    rollback_script=f"ALTER TABLE {safe_table2} ALTER COLUMN {safe_col2} DROP NOT NULL;",
                    dependencies=[],
                    validation_query=f"SELECT COUNT(*) FROM {safe_table2} WHERE {safe_col2} IS NULL;"
                ))
    ...
    def _analyze_constraint_changes(self, table_name: str, before_constraints: Dict[str, Any], 
        ...
        for constraint_type in ["primary_key", "foreign_key", "unique", "check"]:
            ...
            for constraint in after_set - before_set:
                ...
                safe_table3 = _validate_identifier(table_name)
                scripts.append(MigrationScript(
                    script_type="sql",
                    description=f"Add {constraint_type} constraint to {table_name}",
                    script_content=f"ALTER TABLE {safe_table3} ADD CONSTRAINT {constraint_type}_{table_name} {constraint_type.upper()} ({constraint});",
                    rollback_script=f"ALTER TABLE {safe_table3} DROP CONSTRAINT {constraint_type}_{table_name};",
                    dependencies=[],
    ...
    def _generate_column_definition(self, col_name: str, col_def: Dict[str, Any]) -> str:
        ...
        return f"{safe_col} {col_type}{nullable}{default}"
...
def main():
    ...
    try:
        ...
        if args.format in ["text", "both"]:
            ...
            text_output = str(Path(args.output).with_suffix('.txt')) if args.output else None
            if text_output:
