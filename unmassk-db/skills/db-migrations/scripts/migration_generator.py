...
class MigrationGenerator:
    ...
    def _create_table_step(self, table: Table) -> MigrationStep:
        ...
        columns_sql = []
        safe_table_name = _validate_identifier(table.name)

        for col_name, column in table.columns.items():
            safe_col_name = _validate_identifier(col_name)
            col_sql = f"{safe_col_name} {column.data_type}"
        ...
        if table.primary_key:
            pk_cols = ', '.join(_validate_identifier(c) for c in table.primary_key)
            pk_sql = f"PRIMARY KEY ({pk_cols})"
            columns_sql.append(pk_sql)

        # Add foreign keys
        for col_name, ref in table.foreign_keys.items():
            safe_fk_col = _validate_identifier(col_name)
            fk_sql = f"FOREIGN KEY ({safe_fk_col}) REFERENCES {ref}"
    ...
    def _add_column_step(self, col_info: Dict[str, Any]) -> MigrationStep:
        ...
        safe_col = _validate_identifier(column.name)

        col_sql = f"{safe_col} {column.data_type}"
        ...
        add_sql = f"ALTER TABLE {safe_table} ADD COLUMN {col_sql};"
        drop_sql = f"ALTER TABLE {safe_table} DROP COLUMN {safe_col};"
    ...
    def _modify_column_step(self, col_info: Dict[str, Any]) -> MigrationStep:
        ...
        if current_def.data_type != target_def.data_type:
            alter_statements.append(f"ALTER COLUMN {safe_col} TYPE {target_def.data_type}")
            rollback_statements.append(f"ALTER COLUMN {safe_col} TYPE {current_def.data_type}")

        if current_def.nullable != target_def.nullable:
            if target_def.nullable:
                alter_statements.append(f"ALTER COLUMN {safe_col} DROP NOT NULL")
                rollback_statements.append(f"ALTER COLUMN {safe_col} SET NOT NULL")
            else:
                alter_statements.append(f"ALTER COLUMN {safe_col} SET NOT NULL")
                rollback_statements.append(f"ALTER COLUMN {safe_col} DROP NOT NULL")

        if current_def.default_value != target_def.default_value:
            if target_def.default_value:
                alter_statements.append(f"ALTER COLUMN {safe_col} SET DEFAULT {target_def.default_value}")
            else:
                alter_statements.append(f"ALTER COLUMN {safe_col} DROP DEFAULT")

            if current_def.default_value:
                rollback_statements.append(f"ALTER COLUMN {safe_col} SET DEFAULT {current_def.default_value}")
            else:
                rollback_statements.append(f"ALTER COLUMN {safe_col} DROP DEFAULT")

        # Build SQL
        alter_sql = f"ALTER TABLE {safe_table}\n    " + ",\n    ".join(alter_statements) + ";"
        rollback_sql = f"ALTER TABLE {safe_table}\n    " + ",\n    ".join(rollback_statements) + ";"
    ...
    def _modify_column_zero_downtime_steps(self, col_info: Dict[str, Any]) -> List[MigrationStep]:
        ...
        safe_temp_col = _validate_identifier(temp_column)
        ...
        step1 = MigrationStep(
            ...
            sql_forward=f"ALTER TABLE {safe_table} ADD COLUMN {safe_temp_col} {target_def.data_type};",
            sql_rollback=f"ALTER TABLE {safe_table} DROP COLUMN {safe_temp_col};",
            zero_downtime_phase="EXPAND"
        )
        steps.append(step1)
        ...
        step2 = MigrationStep(
            ...
            sql_forward=f"UPDATE {safe_table} SET {safe_temp_col} = {safe_col};",
            sql_rollback=f"UPDATE {safe_table} SET {safe_temp_col} = NULL;",
            zero_downtime_phase="EXPAND"
        )
        steps.append(step2)
        ...
        step3 = MigrationStep(
            ...
            sql_forward=f"ALTER TABLE {safe_table} DROP COLUMN {safe_col};",
            sql_rollback=f"ALTER TABLE {safe_table} ADD COLUMN {safe_col} {current_def.data_type};",
    ...
    def _add_constraint_step(self, constraint_info: Dict[str, Any]) -> MigrationStep:
        ...
        if constraint_type == 'PRIMARY_KEY':
            ...
            safe_cols = ', '.join(_validate_identifier(c) for c in columns)
            add_sql = f"ALTER TABLE {safe_table} ADD CONSTRAINT {constraint_name} PRIMARY KEY ({safe_cols});"
            drop_sql = f"ALTER TABLE {safe_table} DROP CONSTRAINT {constraint_name};"
            ...
            safe_cols = ', '.join(_validate_identifier(c) for c in columns)
            add_sql = f"ALTER TABLE {safe_table} ADD CONSTRAINT {constraint_name} UNIQUE ({safe_cols});"
            drop_sql = f"ALTER TABLE {safe_table} DROP CONSTRAINT {constraint_name};"
            ...
            add_sql = f"ALTER TABLE {safe_table} ADD CONSTRAINT {constraint_name} CHECK ({condition});"
            drop_sql = f"ALTER TABLE {safe_table} DROP CONSTRAINT {constraint_name};"
    ...
    def _add_index_step(self, index_info: Dict[str, Any]) -> MigrationStep:
        ...
        safe_table = _validate_identifier(table)
        safe_index_name = _validate_identifier(index['name'])

        unique_keyword = "UNIQUE " if index.get('unique', False) else ""
        columns_sql = ', '.join(_validate_identifier(c) for c in index['columns'])

        create_sql = f"CREATE {unique_keyword}INDEX {safe_index_name} ON {safe_table} ({columns_sql});"
        drop_sql = f"DROP INDEX {safe_index_name};"
    ...
    def _rename_table_step(self, rename_info: Dict[str, Any]) -> MigrationStep:
        ...
        safe_old = _validate_identifier(old_name)
        safe_new = _validate_identifier(new_name)

        rename_sql = f"ALTER TABLE {safe_old} RENAME TO {safe_new};"
        rollback_sql = f"ALTER TABLE {safe_new} RENAME TO {safe_old};"
    ...
    def _drop_table_step(self, table_info: Dict[str, Any]) -> MigrationStep:
        ...
        drop_sql = f"DROP TABLE {safe_table_name};"
