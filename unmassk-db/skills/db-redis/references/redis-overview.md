...
Comprehensive performance optimization guide for Redis, including Redis Query Engine, vector search, and semantic caching. 35 rules across 11 categories, prioritized by impact.
...
- `data-incr` - Use INCR for Atomic Counters
- `data-transactions` - Use Transactions for Atomic Multi-Command Operations
- `data-hash-field-expiry` - Use Hash Field Expiration for Per-Field TTL
...
- `conn-client-cache` - Use Client-Side Caching for Frequently Read Data
...
- `rqe-skip-initial-scan` - Use SKIPINITIALSCAN for New Data Only Indexes
