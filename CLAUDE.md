...
- **Where the web-app material actually lives: `unmassk-audit`.** Its 9 agent prompts still hardcode `npx vitest`, `npx prettier`, `backend/src/[MODULE]/`, Zod and a 97% coverage bar, while the skill's own description claims to be stack-agnostic. **That contradiction is open and unresolved** — treat any audit of this Python toolkit run through those prompts as producing commands that cannot execute here, until it is fixed.
