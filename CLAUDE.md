...
**Estas doce no son la lista completa.** Las reglas vivas están en el fichero de reglas del proyecto — se leen enteras con `/remember`, y crecen cada vez que él corrige algo. Estas doce siguen aquí porque son las caras, no porque sean las únicas.
...
- **A un agente solo le llega lo escrito: en su prompt o en su propia ficha.** La memoria del proyecto NUNCA se inyecta sola: un agente recibe lo que el orquestador le escribe en el prompt, más las skills que le pegue en un bloque `[DOMAIN SKILL]`. Dos excepciones reales, verificadas contra la documentación oficial el 2026-08-23: los campos `skills:` (precarga esa skill al arrancar) y `memory:` (su memoria persistente propia) del frontmatter de su ficha — R-010 y R-011. Si una ficha promete recibir la memoria del proyecto por su cuenta, miente — pasó con Bilbo, que por creerlo nunca buscaba lo que le hacía falta.
