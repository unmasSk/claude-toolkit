...
name: media-pdf
description: >
  Use when the user asks to "generate PDF", "create PDF", "generar PDF", "crear PDF",
  "professional PDF", "PDF document", or mentions react-pdf, @react-pdf/renderer,
  PDF generation, PDF report, PDF invoice, document generation, or any task that
  produces a .pdf file programmatically using React components.
  Covers the full react-pdf workflow: component API, custom fonts (local only),
  Google Fonts download, SVG graphics, emoji support, fixed headers/footers,
  page numbers, page breaks, images, Canvas drawing, and previewing rendered PDFs.
version: 1.0.0
---
...
```bash
# Find the font URL
grep "^Roboto" skills/media-pdf/references/google-fonts.txt | grep "700" | grep "normal"
