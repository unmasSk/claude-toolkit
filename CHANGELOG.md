...
- **Windows: el fixture de objeto git corrupto fallaba con `autocrlf` activo**, que relee y reprocesa el blob al hacer `git add` — su premisa de "solo pasa en POSIX" no se sostenía. El path queda exento vía `.git/info/attributes -text` (M-127).
- **Ficha de House: el `CLEANUP CHECK` podía fallar la puerta para siempre si el repo tenía un binario rastreado**, que `grep` leía como texto y podía coincidir sin forma de descartarlo. Añadido `-I` para ignorar binarios.
- **Barrido de referencias muertas tras la migración v1 → gitmem**: 5 referencias a nombres y escritores ya borrados corregidas en docstrings/comentarios de `bin/` y `lib/`, para que la próxima lectura no asuma un sistema que ya no existe.
- **Suite de tests realineada con el sistema real**: dos tests probaban ramas muertas —un subprocess a un script ya borrado, y un formato de trailers que este proyecto ya no escribe— sin ejercitar código real del proyecto. Uno se reescribió contra un fallo real de git, el otro se retiró.
