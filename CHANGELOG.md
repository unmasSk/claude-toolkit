...
### Fixed
- **CI de Ubuntu con 37 tests en rojo**: el filtro que fabrica un `PATH` sin `gh` real para los tests de `test_work_issue_field.py` descartaba el directorio entero que contuviera `gh` — correcto cuando `gh` y `git` viven en sitios distintos (macOS local), pero en `ubuntu-latest` ambos conviven en `/usr/bin`, así que quitar el directorio se llevaba `git` por delante. Ahora filtra por fichero, no por directorio: reconstruye ese directorio en un scratch con un symlink a cada entrada excepto `gh`/`gh.exe`/`gh.cmd`/`gh.bat`, así que `git` y cualquier otro binario que comparta carpeta con `gh` sigue siendo localizable. Test de regresión permanente añadido (`test_conftest_path_without_real_gh.py`).
- **Windows: los binarios sintéticos de los tests de `gh` no eran localizables.** `shutil.which()` en Windows solo resuelve nombres con una extensión de `PATHEXT` (`.exe`, `.cmd`…); los fixtures creaban un `gh` a secas, así que la búsqueda fallaba en esa plataforma. Ahora se crean con `.exe`.
- **Windows: el fixture de objeto git corrupto fallaba con `autocrlf` activo.** Con `core.autocrlf=true`, `git add` relee y reprocesa el blob desde el índice — su premisa de "solo pasa en POSIX" no se sostenía. El path del fixture queda exento vía `.git/info/attributes -text`.
- **Ficha de House: el `CLEANUP CHECK` (`git grep -n --untracked ...`) podía fallar la puerta para siempre si el repo tenía un binario rastreado** — `grep` trataba su contenido como texto y podía coincidir por accidente sin forma de descartarlo. Añadido `-I` (ignora binarios) al comando canónico.
- **Barrido de referencias muertas tras la migración v1 → gitmem**: 5 referencias corregidas en docstrings/comentarios de `bin/`, `lib/` (renombrado `gitmemory`, escritores ya borrados), y una segunda pasada en `_symlink_safe_open.py`, `boot_health.py`, `hooks_doc.py`, `parsing.py`, `upgrade_check.py` (hooks v1 borrados que todavía se citaban en comentarios).
- **Suite de tests realineada con el sistema real**: `TestT26NoDuplicateStderr` (`test_release.py`) probaba una rama que ya no se disparaba nunca — interceptaba un subprocess a `git-memory-commit.py`, borrado hace tiempo; el mecanismo real es `notes.write_work()` llamado como función Python directa. Reescrito para forzar un fallo real de git (`.git/index.lock` plantado) en vez de un doble simulado. `test_drift.py::test_deep_search` (y sus generadores exclusivos) retirado: generaba 200 commits con el formato de trailers v1 (`Decision:`, `Memo:`) que este proyecto ya no escribe — el productor real es `lib/memory/format.py`, con vocabulario de campos distinto (`Why`, `Keys`, `Description`…) — y el test nunca llamaba a código del proyecto, solo probaba que `git log --grep` encuentra una cadena que el propio test insertó.

### Added
- **`BootSummary.issues`**: el informe de arranque ya no solo cuenta cuántas issues distintas llevan nota viva — lista cada una, una por línea, con su número de issue y el titular de su propia nota (`lib/memory/model.py`, `lib/memory/boot.py`), que es lo que la fila "Issues" del menú de apertura (D-060/D-064) necesitaba para pintarse y antes no tenía de dónde sacar. Contrato de tests dedicado: `tests/memory/test_boot_issues_field.py` (orden, caso-cero byte-idéntico al anterior, notas archivadas excluidas).

### Changed
- `unmassk-memory`: el menú de apertura de sesión es una tabla fija de dos columnas con ocho secciones siempre presentes, en este orden — Today, Last session, Blockers, Questions, Incidents, Branch, Issues y Health — un elemento por línea, etiqueta solo en la primera fila de cada sección, guion en la sección vacía, escrita en el idioma del usuario, cerrada con una línea que ofrece por dónde empezar. Branch muestra siempre la rama actual, con una línea extra si difiere de la última trabajada y otra si hay commits sin subir. Health resume los chequeos de coherencia del arranque: una línea si todo está en verde, o cada fallo en su propia línea, sin suavizar y siempre primero. Los muros dejan de listarse ahí: Claude los sigue leyendo y aplicando, pero solo se mencionan al usuario en una línea si bloquean directamente lo que va a hacer; si el usuario pide verlos, se presentan aparte en una tabla limpia y en llano, nunca como volcado de ids y zonas. Sustituye al diseño anterior, donde Health era una sexta fila condicional que solo aparecía cuando algo fallaba.
- **Ficha de Alexandria (`unmassk-toolkit/agents/alexandria.md`)** gana la regla de markdown a ancho completo — una línea por párrafo o viñeta, nunca cortada a mano por columna, aplicable a todo lo que escribe (CHANGELOG, README, docs/, CLAUDE.md) — y la exige explícitamente para las entradas del propio CHANGELOG, que ya venían así en la práctica.
- **Formato del propio CHANGELOG (D-067)**: pasa a Keep a Changelog como base con tres desviaciones — `Fixed` va primero, una entrada nacida de una issue la enlaza al final, y una sección `Known` cierra lo que se publica roto y conocido — fijado en la ficha de Alexandria y aplicado en este fichero desde esta entrada.
...
- **Nueva referencia `references/issues.md` en la skill de memoria**: cuándo una nota pide trabajo en vez de solo guardar un hecho, la raya entre arreglar ya y proponer una issue, y las etiquetas de las issues — tres prioridades, el tipo, y las dos zonas de la nota que la parió. Una issue sigue siendo una decisión del propietario: se propone, nunca se abre por cuenta propia.
...
- **Una búsqueda que no encuentra nada ya no es un callejón sin salida: enseña el vocabulario del proyecto.** `gitmem search` es una búsqueda literal de letras, así que un cero no significa «no hay memoria de esto» sino «esa cadena no está escrita en ninguna parte» — y las dos cosas salían idénticas por pantalla. Ahora, cuando no hay resultados, la salida incluye las zonas del proyecto con su descripción y sus alias, que son las palabras que ese proyecto sí usa, y quien busca acierta a la segunda. Si el proyecto todavía no tiene zonas, lo dice en vez de enseñar una lista vacía — que es lo que separa «no hay nada de esto» de «aquí aún no hay memoria». Una búsqueda con resultados no cambia en nada.
  Salió de un caso real: un agente hizo cuatro búsquedas por nombre de fichero, se llevó cuatro ceros y se puso a leer código, teniendo al lado 18 notas de la zona exacta de su tarea, con los dos muros que le aplicaban y el bloqueador de lo que estaba investigando. Su ficha ya le decía que reintentara con la palabra del proyecto; no lo hizo. Por eso el arreglo va en la herramienta, que todos tocan, y no en las nueve fichas, que hay que acordarse de cumplir.
