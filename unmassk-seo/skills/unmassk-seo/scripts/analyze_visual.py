...
def main():
    ...
    if args.json:
        ...
        print("\nAbove the Fold:")
        af = result["above_fold"]
        print(f"  H1 Visible: {'Yes' if af['h1_visible'] else 'No'}")
        print(f"  CTA Visible: {'Yes' if af['cta_visible'] else 'No'}")
        print(f"  Hero Image: {af['hero_image'] or 'None found'}")

        print("\nMobile Responsiveness:")
        mob = result["mobile"]
        print(f"  Viewport Meta: {'Yes' if mob['viewport_meta'] else 'No'}")
        print(f"  Horizontal Scroll: {'Problem detected' if mob['horizontal_scroll'] else 'OK'}")
