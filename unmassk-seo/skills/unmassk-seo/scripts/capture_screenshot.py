...
def main():
    ...
    cwd = os.getcwd()
    plugin_root = os.environ.get("CLAUDE_PLUGIN_ROOT", cwd)
    home = os.path.expanduser("~")
    if not (output_dir.startswith(cwd) or output_dir.startswith(home) or output_dir.startswith(plugin_root)):
        print("Error: Output path must be within current directory, home directory, or plugin root", file=sys.stderr)
        sys.exit(1)
