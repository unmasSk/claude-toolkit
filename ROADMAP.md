...
- **Prompt-improver / "superprompt"** — herramienta/skill que mejore MIS prompts (los de Bex) antes de que Claude actúe. Dos sabores encontrados (búsqueda web, sin verificar aún): (a) **HOOK** que caza prompts vagos, investiga el repo y hace 1-6 preguntas concretas antes de ejecutar — [severity1/claude-code-prompt-improver](https://github.com/severity1/claude-code-prompt-improver); ≈ lo que ya hace `unmassk-grill` pero **automático** + anclado al repo; (b) **SKILL que REESCRIBE** el borrador en un prompt afinado (marco "socio senior") — [wagnersza/prompt-improver](https://github.com/wagnersza/prompt-improver); **esto NO lo tenemos** y es lo que Bex describía. Además: prompt-improver oficial de Anthropic (consola), Langfuse (Agent Skills, 4 fases). _A investigar a fondo (Bex no lo tenía claro aún) — ver el Next de cierre. En investigación._
