...
**Keep enabled for:** operators/controllers, in-cluster config (`rest.InClusterConfig()`), service mesh sidecars needing API access.

**Disable for:** web applications, APIs, stateless services, batch jobs without cluster API interaction.
