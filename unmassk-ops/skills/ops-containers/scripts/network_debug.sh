...
# Validate POD_NAME and NAMESPACE against safe Kubernetes name pattern
if [[ ! "$POD_NAME" =~ ^[a-z0-9]([a-z0-9.-]*[a-z0-9])?$ ]]; then
    echo "ERROR: Invalid pod name '${POD_NAME}'. Must be a valid RFC 1123 label" >&2
    exit 1
fi
if [[ ! "$NAMESPACE" =~ ^[a-z0-9]([a-z0-9.-]*[a-z0-9])?$ ]]; then
    echo "ERROR: Invalid namespace '${NAMESPACE}'. Must be a valid RFC 1123 label" >&2
    exit 1
fi

# Validate REQUEST_TIMEOUT format (e.g. 15s, 2m, 1h)
if [[ ! "$REQUEST_TIMEOUT" =~ ^[0-9]+[smh]$ ]]; then
    echo "ERROR: Invalid REQUEST_TIMEOUT '${REQUEST_TIMEOUT}'. Must match ^[0-9]+[smh]$ (e.g. 15s, 2m, 1h)" >&2
    exit 1
fi
