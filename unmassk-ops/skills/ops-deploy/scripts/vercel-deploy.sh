...
# Deploy
echo "Deploying..." >&2
HTTP_CODE=$(curl -s -o /tmp/vercel-deploy-response.json -w "%{http_code}" -X POST "$DEPLOY_ENDPOINT" -F "file=@$TARBALL" -F "framework=$FRAMEWORK")
RESPONSE=$(cat /tmp/vercel-deploy-response.json 2>/dev/null || echo '{}')
rm -f /tmp/vercel-deploy-response.json

# Check HTTP status first
if [ "$HTTP_CODE" -lt 200 ] || [ "$HTTP_CODE" -ge 300 ]; then
    echo "Error: Deploy API returned HTTP $HTTP_CODE" >&2
    echo "$RESPONSE" >&2
    exit 1
fi
