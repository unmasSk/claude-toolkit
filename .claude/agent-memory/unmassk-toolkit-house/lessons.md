...
## Lesson: on Windows the missing `encoding=` on a subprocess read is NOT the same failure as on POSIX — same line, two different symptoms, and only one of them is loud

`subprocess.run(..., text=True)` with no `encoding=` reading a child that emits UTF-8: on **POSIX** `_communicate` decodes in the MAIN thread, so `UnicodeDecodeError` propagates to the caller — and a `try: ... except Exception: pass` wrapper swallows it whole (silent, nothing printed, the rest of the `try` block skipped). On **Windows** `_communicate` decodes inside `_readerthread`, so the exception dies in the thread, `threading.excepthook` dumps a full Traceback onto the process's own stderr (uncatchable by the caller), the buffer stays empty and `stdout = stdout[0] if stdout else None` hands back **`None`** — the caller keeps running with a `None` where it expected a string. Verified against the CPython source and reproduced byte-for-byte locally by writing `encoding="cp1252"` explicitly, which is exactly what bare `text=True` resolves to on the CI runner.

**How to apply:** to reproduce a Windows cp1252 decode failure from macOS, do NOT fight the locale (`LC_ALL=C` + `PYTHONCOERCECLOCALE=0` still yields UTF-8 on macOS/py3.14 — tried, no reproduction). Capture the child's raw **bytes** and call `.decode("cp1252")`: that is literally the operation `_readerthread` performs, and it reproduces the exact codec, byte and message. Then audit every consumer of that call for `None`-tolerance — `json.loads(None)`, `None.strip()`, `x in None` are the three that turn an ugly traceback into a real crash.
