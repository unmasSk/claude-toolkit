...
## Pattern: The three memory static-gates fail on NEW code, not on a mechanism bug — read them as "the gate caught construction the docstring predates"

**Project:** unmassk-toolkit (tests/memory/test_boundary.py + test_rejection_relaunch_commands.py) · **Seen:** 2026-08-06 · confirmed by AST trace, no instrumentation

Three permanent static gates in `tests/memory/`. When one goes red after new work, the gate is almost always RIGHT and the new code is the outlier. Each has a precise, non-obvious contract worth knowing before diagnosing:

1. **`test_no_file_outside_the_allowed_zone_imports_lib_memory` (Sec.13 row 1).** Allowed zone = `lib/memory/`, `bin/memory/`, `bin/gitmem`, `hooks/customs.py`, `hooks/boot_launcher.py`, `tests/memory/`. ANY other toolkit `.py` that imports a `lib/memory/` stem (AST `Import`/`ImportFrom`, comments don't count) is flagged. Purpose: v2 must be deletable without breaking boot OR the installer. Trap: a NEW file in `lib/` (not `lib/memory/`) that legitimately reuses a memory function (e.g. `lib/install_apply.py` calling `indexes.seed` to seed project-memory) collides head-on with this. It is a policy collision (deliberate Sec.34 reuse vs deletability invariant), NOT an accidental bug — route to owner: either decouple (installer shells out / stays outside lib/memory) or widen the zone (weakens deletability, owner-approval only).

2. **`test_no_public_symbol_has_zero_production_and_zero_tests` (Sec.13 row 3, symbol half).** Two-branch AST scan per public symbol. Production branch DELIBERATELY excludes same-file callers (`if owner_stem == own_stem: continue`) — it wants a CROSS-FILE consumer. Rule: production==0 AND tests==0 → red. Trap: a public function whose ONLY caller is a same-file sibling (e.g. `health.memory_mounted`/`possible_unconverted_legacy` called only at health.py by `build()`) counts as production 0; if no test calls it directly either, it goes red. This is NOT dead code and NOT a scanner blind spot — it means the symbol never needed to be public. Fix = privatize with `_` prefix (precedent: `boot.blockers_section`→`_blockers_section`, cited in the test docstring), OR give it a direct test. The tests branch resolves fixture aliases (`fmt` for `format`), module-level `import_lib_memory_module` vars, and methods inside `class Test...:` — so "0 tests" is trustworthy.

3. **`test_relaunch_command_flags_and_required_args_match_real_argparse`.** Harness `_real_parser_for_subcommand` monkeypatches `ArgumentParser.parse_args` to return a bare `argparse.Namespace()`, then calls `module._parse_args([])` to capture the real parser WITHOUT parsing. This assumes every `bin/memory/<sub>.py::_parse_args` is a thin `return parser.parse_args(argv)`. Trap: `rule.py:_parse_args` is the ONLY one of 10 siblings that post-processes the result (`if args.text is not None:` normalizing list/ls) — under the spy's empty Namespace that raises `AttributeError: 'Namespace' object has no attribute 'text'`. Production works fine in real use; the failure is a convention violation (PIEZAS.md Sec.10: no logic in the CLI script) exposed only by the harness. Fix = move the post-parse normalization out of `_parse_args` into `main`, making `_parse_args` thin like its siblings (Ultron, preferred, aligns 10:1 convention); or harden the spy (Dante, but it can't produce real values).

**Lesson:** these three are structural gates, diagnosable by reading + AST, NEVER by instrumentation (no `[HOUSE:]` needed). When red, compare the failing symbol/file against the gate's documented green state (the test docstrings carry dated "estado real" snapshots) and against git log on the offending file — the newer side is the outlier. A gate firing is the gate WORKING.
...
## Pattern: before blaming an uncommitted diff, prove HEAD itself still works — and never trust a partial pytest run in this suite

**Project:** unmassk-toolkit (feat/memoria-v2, memory-v1 amputation of the 5 boot files) · **Seen:** 2026-08-02 · confirmed by direct import of HEAD blobs + 3 live hook runs + full-vs-subset pytest comparison

Two traps that both produce a confident-but-wrong verdict on a large in-flight refactor:

1. **HEAD may already be broken, so the diff under review can be the REPAIR, not the damage.** In this branch `lib/boot_memory.py` was deleted in commit `177420b` while `lib/boot_render.py` kept `from boot_memory import MAX_DECISIONS, _crown_replace` at module level — and `hooks/session-start-boot.py` imports boot_render at module level. So SessionStart had been dying with `ModuleNotFoundError` on every session for 5 commits. Cheap probe that avoids a worktree: `git show HEAD:<path>` each module into a tmpdir, then `python3 -c "sys.path.insert(0,TMP); sys.path.insert(0,'lib'); import X"`. **Order matters** — insert the tmpdir LAST so it wins; getting it backwards silently imports the working-tree copy and yields a false "HEAD OK".

2. **Running a subset of `tests/test_*boot*` alone produces ~68 phantom failures.** `tests/test_migrate_statusline.py` deliberately stubs `git_helpers` into `sys.modules` during its own import window; whether a stably-named module (`boot_health`, `boot_render`, `boot_git_checks`) gets frozen to that stub depends on collection order. Proof: `test_issue64_boot_health_semver_comparison.py` = 5 failed in the boot subset, 5 PASSED when run alone. **Only the full-suite run is authoritative** — and it needs `--continue-on-collection-errors`, because a single module-level ImportError in any test file makes pytest abort with "Interrupted: N errors during collection" and run ZERO tests (a mid-refactor suite is therefore not red, it is absent — which reads identically to "nobody verified this").
