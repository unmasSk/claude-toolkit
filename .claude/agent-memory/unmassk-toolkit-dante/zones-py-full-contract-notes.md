...
description: lib/memory/zones.py + bin/memory/zones.py full campaign merged from 4 date-split files — original §6.2 contract + cross-import infra fix, English rename + duplicate bounce, alias-collision bounce, absent-vs-empty doctor gap
metadata:
  type: project
---

Merged 2026-08-25 (memory compaction pass, phase 2) from 4 separate files that all covered the SAME piece of
code — `lib/memory/zones.py` / `bin/memory/zones.py` — split only by which session touched it. Per this
...
`zones-contract-notes.md`, `zones-script-english-rename-and-duplicate-bounce-notes.md`,
`zones-alias-collision-bounce-contract-notes.md`, `zones-list-doctor-absent-vs-empty-contract-notes.md`.
...
## Round 4 (2026-08-06) — zones.py list + doctor.py: absent-vs-empty zones.json masking
