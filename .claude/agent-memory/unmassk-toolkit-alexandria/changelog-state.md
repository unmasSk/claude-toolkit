...
Last full audit: 2026-07-04 (fourth pass same day)
Last commit covered (toolkit root): 4042f28 (docs(plugin/roadmap): note the small startup quality-floor piece is done; roadmap-order rule note; commit local editor color pref) — HEAD at time of this audit, working tree otherwise clean.
...
Current version in plugin.json: v1.14.0 released (chore acd0d01). [Unreleased] filled again (2026-07-04) with the one real product change from the two commits that landed after that release (278b41b, 4042f28) — version bump is NOT Alexandria's job, that's release.py's.
...
**How to apply:** On next run: `git log 4042f28..HEAD --oneline` for toolkit root; `git log f4196fa..HEAD --oneline -- chatroom/` for chatroom. Check for new code changes not yet in either CHANGELOG.md.
