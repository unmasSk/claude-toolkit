...
**How to apply:** On next run: `git log b3dd092..HEAD --oneline` for toolkit root (supersedes the old `80cf97e..HEAD` marker — this pass's #55 entry already covers everything through b3dd092); `git log f4196fa..HEAD --oneline -- chatroom/` for chatroom. Check for new code changes not yet in either CHANGELOG.md. Note: #49 (multi-machine boot freshness) is already released as [1.17.0] — confirmed in CHANGELOG.md before writing this pass, not stale. [Unreleased] now holds only the fresh #55 date-parsing entries (Added + Fixed), waiting on the next squash/`release.py` run. Also check `git status`/`git diff HEAD` for uncommitted working-tree changes, not just `git log`.
