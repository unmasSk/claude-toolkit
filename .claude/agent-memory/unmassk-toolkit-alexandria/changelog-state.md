...
**How to apply:** On next run: `git log 51a3c44..HEAD --oneline` for toolkit root; `git log f4196fa..HEAD --oneline -- chatroom/` for chatroom. Check for new code changes not yet in either CHANGELOG.md. Note: [Unreleased] is not yet versioned — next release.py run will stamp it as v1.16.1. Also check `git status`/`git diff HEAD` for uncommitted working-tree changes, not just `git log` — this pass's fix was documented before Gitto committed it.
