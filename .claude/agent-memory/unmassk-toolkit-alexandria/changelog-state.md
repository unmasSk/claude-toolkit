...
Last full audit: 2026-07-05 (fifth pass — big one, 14-round security/quality pipeline on the boot hook)
Last commit covered (toolkit root): f6cc6ac (decision: git-memory-upgrade.py 537-line LOC exception accepted) — HEAD at time of this audit, working tree otherwise clean.
...
**How to apply:** On next run: `git log f6cc6ac..HEAD --oneline` for toolkit root; `git log f4196fa..HEAD --oneline -- chatroom/` for chatroom. Check for new code changes not yet in either CHANGELOG.md. Note: [Unreleased] is not yet versioned — next release.py run will stamp it.
