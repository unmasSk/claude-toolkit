...
**Last commit that actually touched `CHANGELOG.md` on disk: `867fd72` (2026-08-25) — but the working tree now has an uncommitted edit (this pass, 2026-08-26) covering everything through `75fdbbe` (last commit before the pass started; `c04736e` = v1.41.0 release is the range's lower bound).** Content added this pass: `### Added` (`BootSummary.issues` — `lib/memory/model.py`/`boot.py`, feature bundled inside commit `65f47a0` whose message only names an unrelated CI fix; contract tests `test_boot_issues_field.py`); `### Fixed` (CI Ubuntu `path_without_real_gh` 37-red fix `65f47a0`+regression test `4249f6b`, Windows `.exe`/PATHEXT `1b8d027`, Windows autocrlf corrupt-object fixture exemption `9346c8b`, House card `git grep -I` `d9cec70`, v1-sweep dead-reference cleanup `5e1f1f1`+`246d40e`, test suite realignment T26/`deep_search` retirement `75fdbbe`). The pre-existing `### Changed` bullet (8-section opening menu) was verified still faithful against `unmassk-memory/SKILL.md` (now correctly says "eight", the "five" drift flagged 2026-08-25 is resolved) and left untouched.
