...
|------|--------------|--------|
| `CLAUDE.md` (root) | 2026-08-25 | **Close pass (v1.40.0 window, memory legibility+integrity batch)**: 3 claims re-verified, all still true, no edit needed — the "El trabajo rutinario se commitea..." bullet (D-057) was already fixed by a prior commit this session (`48f0053`, clean single-line swap, no stale duplicate left elsewhere); "nueve comandos bajo gitmem" still exact (`bin/gitmem`'s `SUBCOMMANDS` tuple = 9 — the batch added flags to existing commands, `--chain`/`--retract`/`--replaces`, not new subcommands); the memory-system pointer to `unmassk-toolkit/skills/unmassk-memory/` as the single source stands, which is why the gap below lives there and not here. **Real gap found, not fixed** (task LIMITS forbade touching code skills): see stale-zones.md 2026-08-25 entry — `unmassk-memory/SKILL.md` documents 1 of the batch's 4 new capabilities (`unmassk-memory-doctor`), missing `--chain`, the `archivada`/`(↺ id)` markers, and `rule --retract`/`--replaces`. README/docs/root-CLAUDE.md needed no changes this pass — the batch is internal `gitmem` mechanics, no public-facing claim went stale. **Prior pass (2026-08-24, merge-duplicate-rules window, same day, second pass)**: session only merged 2 duplicate entries in `.claude/project-memory/rules.md` (local-only, per this file's own R-051 doctrine — fixes target the toolkit, never this repo's local rules/CLAUDE.md), no code/skill/doc touched. No CLAUDE.md/docs/README update needed. **Prior pass (v1.39.0 window)**: root CLAUDE.md already carried the fix before this pass started (commit `2506c44`, not mine — removed the hand-kept rule count entirely, "estaba en 42, ya son 47", both occurrences) and it now reads correctly with no number to go stale ("que crecen con cada corrección... por eso no se cuentan aquí a mano"). Re-verified against this session's other v1.39.0 changes: R-010/R-011 frontmatter bullet already matches the agent-card rewrite; the two roadmap items ("compactar la memoria de cada agente" / "Yoda, una sola vez") both re-confirmed still open — `gitmem search compactar`/`agent-memory`/`sistema entero` found no closing note. No further edit needed. **Gap found, not fixed (outside close mode's 4-surface table — docs/, module CLAUDE.md, root CLAUDE.md, README — which does not include SKILL.md):** the new program-set checklist mechanism (`hooks/skill-checklist-inject.py` + `checklist-gate.py`, D-052/M-125) has zero mention in `unmassk-close-session/SKILL.md` or `unmassk-council/SKILL.md` — 2 of the 4 skills it gates — even though `checklists/close-session.json` and `checklists/council.json` exist for them. Likely intentional (the whole point of D-052 is that the model does NOT need to know in advance; the hook injects the boxes live), documented instead in CHANGELOG `[1.39.0]` and `gitmem` M-125 — but flag for the owner's call if a static mention is ever wanted. | `CLAUDE.md` (root) | 2026-08-23 | **Close pass**: fixed "37 reglas" → "42 reglas" in both occurrences (lines 15, 38) — `rules.md` grew from 37 to 42 during the session (D-050 quote requirement + 4 more owner corrections, incl. the changelog-ownership rule itself). Added one new "Reglas vivas" bullet: "El CHANGELOG lo escribe Alexandria, nunca el orquestador `[2026-08-23]`" — a genuine coverage gap, root CLAUDE.md had zero mention of who owns the CHANGELOG even though this fact governs this very close pass's mandate (task explicitly said "the owner has ruled that from now on the CHANGELOG is yours"). Considered and NOT added: a bullet for "Dante and Ultron lose the Task tool" — already the canonical Claude-loads-it surface via their own agent-card frontmatter (`tools:` line, verified missing `Task` in both), root CLAUDE.md's existing "Modo test-first" bullet covers their workflow order not their tool grants; adding it here would duplicate the agent cards rather than fill a gap. Everything below "**Fixed 2026-08-09**" is the pre-2026-08-23 history of this same unmanaged intro. Prior pass (2026-08-22): fixed "34 reglas" → "37 reglas" in both occurrences — `rules.md` grew during the session (new no-analogies rule). See stale-zones.md. Everything below this line is the pre-2026-08-22 history of this same unmanaged intro. NOTE 2026-08-02: root `CLAUDE.md` carries an unrelated working-tree edit (not mine, out of scope for that day's task) adding a big recovery block ABOVE the managed markers for the `feat/memoria-v2` branch. It still says "Trece puntos" for `DEUDA.md` — stale again, `DEUDA.md` now has **23 points** (10 closed / 13 open) after the 2026-08-02 full re-verification pass. Flag on next touch — not mine to fix (out of scope, only `DEUDA.md` was authorized that day). **DEUDA.md/the old recovery block no longer exist — the file has since been rewritten as the current, much shorter unmanaged intro (the `docs/deprecated/` framing, "Las broncas", "Cómo se trabajamos aquí", "Reglas vivas"). Fixed 2026-08-09 (close pass)**: line 15's claim "lo único abierto es la issue #83" was stale — `gh issue view 83` confirms CLOSED 2026-08-09 (Bex discarded reinstating message-level memory injection, X-063/DISCARDED.md; the real gap was already closed by zone-word search, D-041/M-115). Rewrote the sentence to state the closure and why, instead of silently dropping the now-false claim. Rest of the unmanaged intro re-verified same pass: "34 reglas" (`grep -c '^\[remember\]' .claude/project-memory/rules.md` = 34, still exact), "protocolo escrito, no ejecutado" for agent-memory compaction (`gitmem search compaction`/`compactar` both 0 hits, still true), "Yoda una sola vez con el sistema entero" still pending (no note found). Managed block below `<!-- BEGIN unmassk-toolkit -->` untouched, generated by `managed_blocks.py`. |
