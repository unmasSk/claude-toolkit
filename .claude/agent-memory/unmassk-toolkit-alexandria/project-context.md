...
- Author: unmasSk
- License: MIT
- Current version: 1.23.0 released (source of truth: unmassk-toolkit/.claude-plugin/plugin.json) — verified 2026-07-26. `[Unreleased]` holds the 1.24.0-bound work in progress: trailer-content validation moved to the wrapper (`bin/git-memory-commit.py`, fail-closed) + the dead trailer-validation hook layer retired (`post-validate-commit-trailers.py` deleted, `pre-validate-commit-trailers.py` cut to its live direct-commit-block only). See changelog-state.md for the full entry breakdown and prior releases' history.
