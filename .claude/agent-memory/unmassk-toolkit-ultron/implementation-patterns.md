...
### Round 3, same day: the rollback itself had two holes — Cerberus caught both before ship (2026-08-08)

**The mistake pattern to remember, independent of the two specific bugs:** I optimized the happy path (commit carries the right content) and treated "undo what shouldn't have been committed" as an afterthought — one unchecked `gitcmd.run(["reset", ...])` call, result discarded, a fixed success string regardless of outcome. Both holes lived in that one line. Any future "undo/rollback" I add anywhere needs the SAME rigor as the forward path: check the result, don't assume the precondition still holds.

- **Hole 1 — unchecked reset result.** `gitcmd.run(["reset", "--mixed", "HEAD~1"], ...)` return value was never even assigned to a variable. If it failed (empty repo — our commit IS the first, no `HEAD~1` to resolve; or `.git/index.lock` held by someone else), the corrupt commit stayed as real `HEAD` while the function still returned the fixed string `"Se deshizo el commit..."` — a lie about the undo, stacked on top of the lie about the content. Fix: capture `reset_result`, branch on `.returncode`, and on failure name the still-live corrupt commit's sha so it can be fixed by hand (never claim "se deshizo" when it didn't).
- **Hole 2 — the graver one: `HEAD~1` resolved AT RESET TIME, not pinned to our own commit.** My original docstring justified this as safe because "the global lock is held the whole time, nobody else could commit in between" — true only for callers who take that SAME lock, and `bin/release.py` (a real publisher) does not. If a legitimate publish lands between our commit and our post-commit verification, `HEAD~1` at reset time is no longer our commit's parent — it's become our own commit, and blind reset eats the other process's real commit. Not a hypothetical: Dante's test forces it deterministically by committing a real `--allow-empty` commit from *inside* the monkeypatched `_committed_blob_hash` (the exact call site `write_work()` uses to verify) and then asserting via real `git log` that the foreign commit survives. Fix: capture `own_commit_sha = git rev-parse HEAD` immediately after `stage_and_commit()` succeeds — **before** calling `_committed_blob_hash()` even once, since that call is precisely where a foreign commit can land. Before attempting any reset, re-read `HEAD` and compare to `own_commit_sha`; on mismatch, touch nothing and fail with a cause that never says "se deshizo" (nothing was undone).
- **Test fixture detail worth remembering**: the `notes_commit` pytest fixture (test_notes.py ~line 108) returns the SAME module object `notes.write_work` reads its globals from (flat import, `from notes_commit import write_work` — same function object, same `__dict__`), so `monkeypatch.setattr(notes_commit, "_committed_blob_hash", fake)` actually affects what `write_work()` calls at runtime. Monkeypatching `notes._committed_blob_hash` would NOT work (that name doesn't exist there — only the 7 explicitly-imported names do). `_empty_repo()` (test_notes.py ~line 224, sibling of `tmp_repo`) is a zero-commit repo — `git init` with nothing else — needed to make `HEAD~1` genuinely unresolvable instead of relying on chance.
- **Non-blocking nit from the same review, fixed anyway (near-zero cost)**: `git hash-object` on the verification tempfile lacked `--no-filters`. The tempfile's path (via `tempfile.mkstemp`) has nothing to do with the real target path being verified, so any `.gitattributes` clean-filter match on the temp path would be accidental and could hash something other than the literal bytes — a false mismatch that rejects a genuinely good commit. This repo has no `.gitattributes` today so it was a latent gap, not an active bug; added `--no-filters` to make "hash of these exact bytes" true unconditionally rather than true-by-coincidence.

### Round 4, same day: `--no-filters` was itself the bug (Windows), and "check-then-act" can never close a live-ref race (2026-08-08)

**`--no-filters` on `git hash-object` was WRONG-DIRECTIONED — it disables the exact normalization git actually applies at commit time.** Round 3's fix reasoned correctly about the risk (a filter resolved against the wrong path) but picked the wrong tool. `stage_and_commit()` commits via ordinary `git add`, which DOES apply path-based filters — on Windows, `core.autocrlf=true` by default normalizes `\r\n`→`\n` when staging. `--no-filters` hashes the raw bytes, so on ANY autocrlf-enabled repo the verification compared raw-CRLF hash against the LF-normalized blob git actually stored — mismatch on every single write, no exceptions. This broke `gitmem wip` in Windows CI (32 tests red across `test_wip_script.py`/`test_rule_script.py`/`test_gitmem_facade.py`) — every valid commit got flagged "corrupt" and rolled back, silently discarding real work. **Confirmed root cause empirically before touching code**: built a scratch repo with `core.autocrlf=true` (this simulates correctly on any OS — autocrlf isn't gated by the host platform, just the config value), committed CRLF content for real, and compared three hashes: `git rev-parse HEAD:<path>` (ground truth), `hash-object --no-filters <tempfile>` (mismatched), `hash-object --path <relpath> <tempfile>` (matched exactly). **The fix: `--path <relpath-of-the-real-target>` instead of `--no-filters`** — `--path` tells `hash-object` "apply the filters this path WOULD get" without requiring the temp file to actually live there; this closes the original round-3 concern (wrong-path filter lookup) AND matches whatever the real commit does (autocrlf today, any future `.gitattributes` rule tomorrow). Verified end-to-end through the REAL `notes.write_work()` (not just raw git commands) in both directions: reverted a scratch copy back to `--no-filters` and reproduced the exact false-positive (valid commit created then wrongly undone) before confirming the `--path` fix doesn't false-positive under the same simulated autocrlf repo.

**Lesson applied generally: a concurrency/verification fix can introduce its OWN platform-specific false positive if the reproduction never leaves the default-config host machine.** macOS/Linux dev boxes don't have autocrlf on by default, so this bug was invisible locally in every test I ran during round 3 — only a Windows CI runner surfaced it. When a fix computes a hash/checksum meant to match what git "really" stores, the verification must be checked against a `core.autocrlf`/`.gitattributes`-active repo, not just a vanilla one, or the fix's own correctness claim is unverified for a large fraction of real users.
