...
## D-056 chain view: an ancestor's own struck line can embed a nested `(↺ grandparent_id)` arrow inside its own `~~...~~` wrapping — not a bug

Confirmed 2026-08-25, D-056 render-lane review (`report_render_chain.py::_note_block`/`_render_thread`). `_status_suffix()` (`report_render_blocks.py`) appends `(↺ {note.replaces})` based purely on `note.replaces` being set, independent of the `archived` bool passed in. In the chain view, `_note_block(ancestor, closed=False)` always passes `archived=False`, but if the ancestor note ITSELF has `.replaces` set (a middle note in a 3+ link chain), its own line still gets the arrow suffix, and the whole line — arrow included — gets wrapped in strikethrough by `_render_thread`. This means the grandparent's id can appear twice in the render: once embedded inside the middle ancestor's own struck arrow, once as its own separately-struck block. `test_search_chain_view.py::_struck()` does a plain regex search for the id inside `~~...~~` anywhere in the output, so this nesting cannot produce a false negative — at worst it's cosmetic redundancy, never a false pass hiding a real gap. Do not flag as a bug; if it recurs, the fix (only worth doing if a human finds the nesting confusing) would be to suppress the replaces-arrow specifically on ancestor lines in chain view, not on live/head lines.

## `kraken workspace promote` really does return exit 1 always — the trading prose is right, do not "correct" it

Confirmed 2026-08-27, unmassk-trading audit. `references/kraken-cli.md:85-87` says `kraken workspace promote <ws>` "returns exit 1 rather than flipping anything — the grading is the value, not the switch". Reading `src/commands/workspace.rs:372` alone this looks wrong: there is a full `Ok(manifest) => ... "promoted": true` arm, so the command *appears* able to flip a paper workspace to live. It cannot: `crates/kraken-workspace/src/app.rs:348-358`'s `pub fn promote()` computes the checklist and then **unconditionally** returns `Err(WorkspaceError::NotPromotable{..})` — the `Ok` arm in the command layer is unreachable dead code upstream. Always follow the call through to the crate before flagging a "the doc understates what this command can do" finding on kraken-cli.

## `price_check.py --pair BTCUSDT` (slashless, 4-char quote) works — the naive `base[:-3]` read is a false alarm

Confirmed 2026-08-27 (EXECUTED, live). `_split_pair("BTCUSDT")` splits into base `"BTCU"` / quote `"SDT"`, which looks like a mis-parse. It is harmless because both fetchers immediately re-concatenate `f"{base}{quote}"` → `"BTCUSDT"`, the venue's own string. The split only exists so `KRAKEN_BASE_ALIASES` can map BTC→XBT, and the only real consequence of a 4-char quote is that the XBT alias silently does not apply (Kraken accepted `BTCUSDT` anyway, verdict OK exit 0). Do not report the split as a parsing bug; the reportable part is only that SKILL.md's "the slashless form also works" has no test behind it.
