...
Lesson for future disambiguation-pass re-audits: don't just re-check the seams the prior audit named — run a full pairwise scan (grep every quoted phrase across every skill's frontmatter, flag any string that repeats verbatim in 2+ skills) BEFORE trusting that "the flagged seams are fixed" means "routing is now clean." A targeted fix pass reliably fixes what was named and reliably leaves unnamed identical phrases untouched.
