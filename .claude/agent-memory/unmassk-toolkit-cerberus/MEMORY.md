...
- [anti-patterns.md](anti-patterns.md) — Recurring shell/Python/TS anti-patterns across boot/git-memory/hooks/CI-flake rounds; latest: `notes_commit.py::write_work()` "verify-then-reset" fix (2026-08-08) — the undo (`git reset --mixed HEAD~1`) itself unchecked + lock-exclusivity assumed against processes outside the lock
- [review-history.md](review-history.md) — Full dated log of past commit-reviews/audits with verdicts, evidence, test counts (2026-07-08 → present). Most recent: 2026-08-08 `notes_commit.py::write_work()` point-8 fix, NOT MERGEABLE (unchecked reset + lying diagnostic on failure, HEAD-moved reset can discard an unrelated commit, zero new tests for the branch)
