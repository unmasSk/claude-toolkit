...
- [anti-patterns.md](anti-patterns.md) — Recurring shell/Python/TS anti-patterns across boot/git-memory/hooks/CI-flake rounds; latest: `notes_commit.py::write_work()` "verify-then-reset", round 2 (2026-08-08) — 3 round-1 holes genuinely fixed, but the reset still targets a live `"HEAD~1"` instead of a pinned SHA, live-reproduced discarding a foreign commit
- [review-history.md](review-history.md) — Full dated log of past commit-reviews/audits with verdicts, evidence, test counts (2026-07-08 → present). Most recent: 2026-08-08 `notes_commit.py::write_work()` point-9 fix round 2 (`94dd863`), NOT MERGEABLE — 4th hole live-reproduced: reset still keyed to dynamic `HEAD~1`, not the checked SHA
