...
3. **Record the test command where the MACHINE reads it.** Write the resolved test command into `.claude/project-memory/config.json` under `test_command` (the same file that holds `repo_type`). This is not just documentation: the `stop-dod-gate` hook **runs `test_command` from that file automatically at session close** — recording it there turns "run the tests" from a line the orchestrator has to remember into a gate that fires by itself (mechanism, not prose). Also save it as a memo, so a person can find it without opening a config file. The gate classifies the result before blocking [2026-08-20]: an intentional test-first red (empty suite, or a first-party module whose file was never written yet) closes the session; a real failure still blocks. `test_command` **runs on every Stop and must be safe to repeat** — no audio, no system notifications, no network calls, no writes outside temp paths; a runner without that guarantee gets its side effects multiplied once per Stop [2026-08-22]. If the working tree is byte-identical to the last Stop, the gate skips rerunning it and replays the prior decision instead. **A red written on purpose while a Dante→Ultron contract is still in flight is not a regression — declare it, don't leave the gate to guess:** `python3 bin/stop-dod-declare.py declare <test_node_id...> --session "$CLAUDE_CODE_SESSION_ID"` shields those exact failing node ids for the rest of the session (`clear` to drop them, `status` to list them); one undeclared failure in the mix still blocks, named, same as before.
