...
- **Skill-injection gate** (`PreToolUse`, matcher `Task` — note the live tool is reported as `Agent`, the matcher catches it): when you invoke a crew subagent (same whitelist as above; Bilbo/Gitto excluded), the hook runs `scripts/skill-search.py` on the task prompt. If a domain skill matches above the confidence bar, it **DENIES the Task** and returns a `⛔ SKILL GATE` reason with one or more `[DOMAIN SKILL — …]` blocks (skill name + path + "read this now"). → **This is EXPECTED, not an error.** Paste the block(s) at the TOP of the agent's prompt and re-invoke the SAME agent (same `subagent_type`, same prompt + the block(s) in front). An anti-loop marker (`[DOMAIN SKILL —` already present in the prompt) lets the retry through. **Fail-open:** searcher failure/timeout/no-match never blocks — the Task proceeds untouched. → This is how domain skills reach agents (they no longer search themselves). Known residual: a prompt densely worded in a domain's vocabulary — even a meta-task *about* that domain — can match the wrong skill; just paste it (harmless — the agent ignores an irrelevant skill) or note to the agent that it doesn't apply.
