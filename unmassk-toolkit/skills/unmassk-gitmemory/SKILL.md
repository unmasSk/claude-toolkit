...
The `[git-memory-boot]` SessionStart hook pre-extracts STATUS, BRANCH, BRANCHES, RESUME, REMEMBER, DECISIONS, MEMOS, TIMELINE, and script paths. Claude does NOT need to run doctor or git-memory-log on boot.

**The boot is a BUDGETED SAMPLE, not the whole memory — never read it as "everything there is".** Hard caps in `lib/boot_memory.py` (`SCAN_DEPTH=30` commits, `MAX_DECISIONS=20`, `MAX_MEMOS=10`, one entry per scope) and `lib/boot_render.py` (`BOOT_MAX_*`). In a mature repo that means single digits out of hundreds — measured on this toolkit: 10 of 287 decisions and 10 of 171 memos, i.e. **96% and 94% never rendered**. The "(N more … in history)" lines are the visible tail of that cut.

**The consequence you must internalise:** an entry's absence from the boot is NO evidence it does not exist. Before telling the user "there is no decision about X", or before acting on anything that might already be decided, run `git-memory-recall.py "<terms>"`. The boot is where you start, not where the memory ends.
...
**TIMELINE scope:** the last 20 commits (was 10), across ALL branches (`git log --all`) — not just the current one — newest first. Lets a session see repo-wide recent activity from any machine/branch without switching. Never renders a commit whose only ref is a confirmed-unrelated upstream (same `exclude_remote` guard as the RESUME path above).

**BRANCHES section:** lists every branch the fetch above just brought from the resolved remote — newest commit first, current branch marked `(current)`, capped at 20 with a "N more not shown" note past that. Comes from the wider fetch, not a separate call; renders nothing if the fetch found no remote (e.g. unrelated upstream, no memory installed).
...
For a full-content rewrite of an existing file (not a read, not an append), pass `atomic=True` to `open_no_follow_symlink()` instead of plain `"w"` mode: a bare `open(path, "w")` truncates the file the instant it's called, so a crash/kill/full-disk error before the new content finishes writing leaves it empty or partial. `atomic=True` writes to a `tempfile.mkstemp()` file in the same directory and only replaces the original via `os.replace()` once the write is fully flushed and fsynced — the original is untouched on any failure before that point, and existing permission bits are preserved across the rewrite. This is what CLAUDE.md's own managed-block writers use (`hooks/session-start-crew.py`, `lib/install_apply.py`, `bin/git-memory-uninstall.py`).

For **concurrent** writers of the same shared file — the lost-update race `atomic=True` does NOT solve, where two writers each read, edit only their own slice, and the last `os.replace()` silently discards the other's change — serialize the entire read-modify-write with `file_lock(target_path, lock_path=None)` (`lib/git_helpers.py`): a cross-platform exclusive lock (POSIX `fcntl.flock`; Windows `msvcrt.locking`, retrying only on the contention errno and re-raising any permanent error rather than looping), lazy platform imports, always released, **not reentrant** (nesting `file_lock` on the same path in one process self-deadlocks). Pass a shared `lock_path` under the already-ignored `.claude/.unmassk/` (see `claude_md_lock_path()`) so the lock file never surfaces in `git status`, and do the cheap "is a write even needed?" check *outside* the lock — only escalate to lock → re-check → write when it is, so a no-op in a read-only dir stays a graceful no-op instead of crashing on lock creation. The 3 CLAUDE.md managed-block writers above use exactly this pattern.
...
**What actually runs.** The table below is *generated* from `hooks/hooks.json` and the files on disk — it cannot drift, and it is the only place in this section that says which hook runs on which event:

<!-- BEGIN unmassk-active-hooks (generated from hooks/hooks.json — do not edit by hand) -->
**12 hook invocations declared** in `hooks/hooks.json`. Event, matcher and timeout are read from that file; file presence is checked on disk; transient measurement probes are excluded. This table is generated — regenerate with `bin/hooks_doc_sync.py --write`, never by editing it here.

| Event | Matcher | Hook file | Timeout |
|---|---|---|---|
| `SessionStart` | — | `session-start-boot.py` | 45s |
| `SessionStart` | — | `session-start-crew.py` | 10s |
| `PreToolUse` | `Bash` | `pre-validate-commit-trailers.py` | 30s |
| `PreToolUse` | `Bash` | `pre-merge-gate.py` | 5s |
| `PreToolUse` | `Bash` | `pre-memory-dedup-gate.py` | 10s |
| `PreToolUse` | `Write\|Edit` | `validate-memory-path.py` | 5s |
| `PreToolUse` | `Task` | `pre-task-recall.py` | 10s |
| `UserPromptSubmit` | — | `user-prompt-memory-check.py` | 10s |
| `Stop` | — | `stop-dod-gate.py` | 90s |
| `Stop` | — | `stop-dod-check.py` | 30s |
| `Stop` | — | `stop-close-session.py` | 10s |
| `PreCompact` | — | `precompact-snapshot.py` | 30s |
<!-- END unmassk-active-hooks -->

**What each one means for you.** Hand-written judgment, deliberately kept outside the generated block: no parser can tell you which hooks are known-dead, which will get in your way, or what to do when one fires. Read it together with the table above — this list explains behaviour, the table owns event/matcher/timeout.

- **Merge gate** (`pre-merge-gate.py`): `git merge` and `git pull` (without `--rebase`) are BLOCKED until Cerberus and Alexandria have reviewed. Bypass once reviewed by appending `# merge-reviewed` to the command. So a merge is never a direct op — launch the reviewers first.
- **Recall gatekeeper — subagents** (`pre-task-recall.py`): before a crew subagent spawns, relevant project memory is auto-injected as a footer on its prompt. The whitelist is the 9 workers — Ultron, Dante, Cerberus, Argus, Moriarty, House, Yoda, Alexandria **and Bilbo** (`hooks/pre-task-recall.py`, `_WORKER_WHITELIST`). **Only Gitto is excluded** — it IS the memory oracle, so feeding recall into it would be circular. The injected block is headed `[PROJECT MEMORY — auto-recalled, relevant to your task]` — that exact label, not any other. → You do NOT need to hand-copy decisions/memos into their prompts; the hook already does it.
- **Domain-skill injection is now the orchestrator's job, not a hook.** The old BM25 `skill-injection gate` (a `PreToolUse` hook that ran `scripts/skill-search.py` and DENIED the Task with a `⛔ SKILL GATE` block) was **removed** — archived at tag/branch `bm25-skill-gate-1.19.9`, recoverable. It produced false positives on long, keyword-dense prompts. Now the orchestrator has every skill's front-matter loaded in its context (via `skillListingBudgetFraction` in settings.json) and pastes the right `[DOMAIN SKILL — …]` block into a crew agent's prompt **by judgment** when the task needs it (see `unmassk-core` → "How to prompt agents"). → If you (a future session) never see a skill gate fire, that is correct — there is no gate. Memory injection (next bullet) is a separate hook and is unaffected.
- **NO per-message memory injection exists for the orchestrator.** ⚠️ This is the single most misread part of the system. The `[memoria relevante para este mensaje]` / `<memory-data>` block was **REMOVED** (decision `1e94975`, issue #69) — it is not "sometimes quiet", it does not exist, and you will never see it. What `user-prompt-memory-check.py` emits on every message is a fixed **banner** (discipline + a pointer to pull recall yourself), plus `[skill-router]` when a trigger phrase matches. That banner is a *reminder*, not a mechanism: **nothing is injected unless you go and fetch it** with `git-memory-recall.py "<terms>"`. The commit that made this change wrote down its own risk verbatim — *"pull depends on Claude's judgement"* — and that risk materialised: the pull almost never happens. Until the channel is rebuilt, YOU are the only thing standing between the memory and its use.
- **Commit gate — ⚠️ CURRENTLY DEAD, do not rely on it.** `pre-validate-commit-trailers.py` is supposed to block direct `git commit`/`git log` and force the wrapper. It has not blocked anything since v1.0.0 (2026-03-17): its guard reads the environment variable `CLAUDE_CODE`, and the real environment exports `CLAUDECODE` (no underscore), so `is_claude` is always false and the hook exits 0 in silence. Verified by `printenv` and by direct use. Its 7 test files pass because `tests/conftest.py:295` fabricates `CLAUDE_CODE=1` — an environment production never provides (a textbook §34 producer/consumer break: the test builds the input the real producer never emits). **Until that one letter is fixed, nothing forces you through the wrapper — use it because it is correct, not because you will be stopped.** Trailer **content** validation, by contrast, IS live and lives in the wrapper `git-memory-commit.py`, not in a hook: it fails **closed** (exit≠0, no commit created) at construction time if a `Memo:`/`Remember:` category is outside its enum (`MEMO_CATEGORIES`/`REMEMBER_CATEGORIES`) or the `category - description` format is malformed / the description sanitises to empty — so an inert memory entry that recall can't retrieve can never be written in the first place. (The former `post-validate-commit-trailers.py` auto-undo hook was retired — it was structurally dead on the wrapper path, since the wrapper's `git commit` runs as a subprocess invisible to the Bash-tool hook; validation at the deterministic producer replaces it.)
- **Memory dedup gate** (`pre-memory-dedup-gate.py`): when you commit a `memo`/`remember`, the near-dup gate compares its text lexically (Jaccard) against existing entries of the SAME type and WARNS — never blocks, fail-open — if it's a near-duplicate, naming the existing entry. → Heed the warning: if it's the same thing reworded, RETIRE the old entry with a `Resolved-Memo`/`Resolved-Remember` tombstone instead of stacking a new one. Decisions are never compared (sacred). Catches lexical near-dups only; semantic restatements still need your judgement.
- **Memory-path guard** (`validate-memory-path.py`): writes to `.claude/agent-memory/` outside the repo root are blocked.
- **Boot + block regen** (`session-start-boot.py`, `session-start-crew.py`): memory is extracted into the boot output, and the **4** managed CLAUDE.md blocks (`unmassk-toolkit`, `unmassk-protocols`, `unmassk-communication`, `unmassk-build-mode`) are regenerated from `lib/managed_blocks.py`. → Editing those managed blocks by hand does NOT persist; change them in the generator.
- **Stop / PreCompact — ⚠️ THEIR OUTPUT NEVER REACHES YOU.** `stop-dod-check.py` (auto-wip + the `[MANDATORY] create a context()` order), `stop-close-session.py` and `precompact-snapshot.py` all run correctly and compute the right thing, then write it to `stderr`/`stdout` with exit 0 — a channel that, on `Stop` and `PreCompact`, goes to the debug log and **not** to your context. Measured over 8,700 real hook executions across 23 sessions: the attachment's `content` field (the only one you see) is populated in `SessionStart` 64/64 and `UserPromptSubmit` 1445/1445, and **0/2506 in `Stop`**, **0/4688 in `PreToolUse`**, with **0 `PreCompact` events recorded at all** despite 3 real compactions. Behavioural proof: that `[MANDATORY]` line has fired 2,506 times and there are 30 `context()` commits in the last 400. → **Closing a session cleanly and checkpointing before compaction are on YOU, not on a hook.** The only channels proven to deliver are `SessionStart`/`UserPromptSubmit` stdout, `PreToolUse` stderr with exit 2, and the legacy `{"decision":"block"}` JSON.
- **Version marker auto-sync** (`session-start-boot.py`, moved off `user-prompt-memory-check.py` in issue #63): once per session start, `needs_upgrade()` compares the project's CLAUDE.md managed-block content against the real canonical render, and its `.claude/.unmassk/manifest.json` version against the plugin code version (numeric SEMVER — 1.10.0 > 1.9.0). If either is outdated, `bin/git-memory-install.py --auto` runs transparently with no output to Claude. You will never see a message for this — it just happens. Fail-safe: missing/corrupt/unparseable manifest → no upgrade, no loop. Downgrade (manifest > code) is ignored. Trade-off of the per-session move: a `/plugin update` mid-session is picked up on the *next* session start, not the very next message.
- **Memory consolidation trigger** (`session-start-boot.py`): if the number of commits since the last `context(consolidation)` reaches the threshold (default 50, overridable via `GIT_MEMORY_CONSOLIDATION_THRESHOLD` env), the boot output emits a `CONSOLIDATE:` block. When you see it, launch Gitto in consolidator mode (Mode C — see `agents/gitto.md`) — the operation is **additive**: Gitto reads the memory, writes crown entries (see Crown below), and closes with a `context(consolidation)` commit that resets the counter. Do not dismiss the block; it means the memory has drifted far enough that a consolidation pass is worth it. Wired end-to-end as of v1.12.0, including the `Retract-Crown:` correction path.
...
A `UserPromptSubmit` hook fires on EVERY user message and prints a fixed banner (discipline + the pull pointer). **There is no `[memory-check]` reminder and no `[memoria relevante…]` block — both were removed in issue #69.** Do not wait for either; if you gate capture on seeing a signal that no longer exists, capture stops happening altogether.

**Evaluate EVERY user message against the list below, unprompted.** The banner is the standing reminder; the trigger is the message itself:
...
- **Validate the plugin before releasing**: run the `plugin-dev:plugin-validator` agent on the plugin directory (spawn it via the Agent tool; if `plugin-dev` is not installed as a subagent type, load its definition from `plugins/plugin-dev/agents/plugin-validator.md` in the `claude-plugins-official` marketplace cache and run a `general-purpose` agent with those instructions). It checks manifest JSON + required fields, kebab-case name, semver, auto-discovery wiring, every `SKILL.md` frontmatter, **that every referenced `references/*.md` file actually exists** (no dangling reference), README/LICENSE presence, no hardcoded secrets, and version coherence between `plugin.json` and the root `marketplace.json`. **MANDATORY for a brand-new plugin** (catches the missing-file / unregistered / version-drift class before it ships); recommended on any bump. Must come back PASS before `bin/release.py`.
...
### Transient Read Resilience (issue #61)

Orthogonal to Passive Healing above: that section is about *what* history is read; this is about surviving a flaky read of whatever history already exists. Every production memory-read site — recall, glossary, timeline, `get_last_context_time()`, `commits_since_last_consolidation()`, `scan_recent_commits()` (bootstrap), and the PreCompact snapshot — calls `git` through `run_git_read_retrying()` (`lib/git_helpers.py`), never `run_git()` directly. A transient `git` failure (non-zero exit, e.g. under CI/runner load) gets up to 3 attempts with a short backoff before the caller's existing fail-safe (empty list/dict/`None`) is returned, so a transient rc≠0 is never silently treated as "no memory here" on the first try. The whole retry sequence is capped to roughly one `GIT_TIMEOUT` in wall-clock time regardless of how the failures land, so a hung `git` process can't multiply a boot's read budget. A genuine empty result (`rc=0`, no output) is never retried.
