...
- **Boot + block regen** (`SessionStart`): memory is extracted into the boot output, and the **4** managed CLAUDE.md blocks (`unmassk-toolkit`, `unmassk-protocols`, `unmassk-communication`, `unmassk-build-mode`) are regenerated from `lib/managed_blocks.py`. → Editing those managed blocks by hand does NOT persist; change them in the generator.
