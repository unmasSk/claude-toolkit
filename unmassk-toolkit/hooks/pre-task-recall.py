#!/usr/bin/env python3
"""PreToolUse hook: inject relevant project memory into Task (subagent) spawns.

Purpose
-------
Intercepts calls to the Task tool. When the spawned subagent is one of the
recognised crew workers and a relevant recall block exists for its prompt,
rewrites the prompt field to append the memory block as a clearly delimited
footer. All other tool calls pass through unmodified.
...
from git operations, or any other exception — is swallowed and results in an
unconditional allow with no updatedInput. A broken recall cannot be allowed
to paralyse the orchestrator. No permissionDecision: "deny" is ever emitted.
