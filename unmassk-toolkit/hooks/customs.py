...
def _find_commit_creating_statement(command):
    ...
    cualquier aparicion textual de git+<subcomando> como candidata SIN
    tokens tras el subcomando -- fail closed (no se puede verificar
    ningun flag, y con la aduana encendida hay que evaluar, no aprobar
    a ciegas). Mismo fallback que
    `pre-validate-commit-trailers.py::_is_direct_git_commit` ya aplica.
    """
    try:
        tokens = shlex.split(command, comments=True)
    except ValueError:
        for sub in _COMMIT_CREATING_SUBCOMMANDS:
