...
silently and prints a compact status/branch briefing that Claude receives
as context.

Memory v2 cleanup (docs/memoria-v2/PLAN-CONSTRUCCION.md §5.3): the memory
extraction (RESUME/REMEMBER/DECISIONS/MEMOS/GC/CONSOLIDATE/TIMELINE
sections, the boot-memory freshness fetch and its MEMORY: stamp) was
removed with the rest of the v1 memory system. What remains — status,
drift, branch, upgrade — is not memory-specific.
...
try:
    # SEC-CRIT-001: symlink-safe writer for the boot log/.gitignore.
    ...
    # T3-1 (Cerberus): shared fallback — see lib/_symlink_safe_open.py for
    # the single implementation this and other entry points share.
...
# old 6000-byte threshold) whose content still fell past the harness's real
...
def write_boot_log(full_text: str, project_root: str | None) -> str | None:
    ...
    never written, silently losing the briefing content — exactly the bug
    this fix exists to prevent (Cerberus regression: TestBootLogWriteFailureFallback).
    """
    if not project_root:
        return None
    try:
        ...
        # git-derived content (commit subjects/branch names — see run_git()'s
        # docstring for how a malformed source could yield a lone surrogate).
        # A clean, always-strict-UTF-8-re-readable escape keeps this
        # write-path's "only OSError escapes" contract even in that case,
        # matching the existing sanitize-for-display discipline this
        # codebase already applies to trailer text (parsing.py's
...
def render_boot_banner_lines(
    ...
    pull_directive_lines: list[str] | None = None,
) -> list[str]:
    ...
    `pull_directive_lines` (issue #49, plan Task 3) is the exception to
    "banner is short, boot-log has everything" — short enough by design to
    fit the banner AND the full boot-log content, and the behind-signal is
    important enough to surface even when Claude never opens the boot-log
    file.
    """
...
def run_preboot_migrations(project_root: str | None) -> None:
    """Run all one-shot pre-boot migrations.
    ...
    layout or have generated JSONs tracked from an old install.
    ...
    Memory v2 cleanup: the hardened, gated, rate-limited background
    `git fetch` for multi-machine memory freshness (issue #49) was removed
    with the rest of the boot-memory freshness system — this function no
...
def main() -> None:
    ...
    (branch_lines, branch, _branch_keywords, _branch_issue, ahead_behind,
     _ahead_n, _behind_n, upstream_ref, pull_directive_lines) = render_branch_section()

    # Shared-history guard (DEUDA.md #6, T2 repo-identity confusion): an
    # `@{u}` that resolves to a coherent, fetchable ref is NOT proof it's a
    # continuation of THIS project's history — a misconfigured
    # branch.<x>.remote/.merge can point at a totally unrelated repo that
    # happens to share a branch name. Run the check once here (only when an
    # upstream_ref exists) and let it gate BOTH surviving upstream-derived
    # outputs below: the PULL DIRECTIVE (git itself would refuse to merge
    # "unrelated histories") and the BRANCHES listing (remote_name is
    # derived from upstream_ref, so nulling it here also empties that
    # section). Nulling upstream_ref reuses render_branches_section()'s
    # existing "no remote" path unchanged, rather than adding a new branch.
    if upstream_ref and check_upstream_shares_history(upstream_ref) is False:
        upstream_ref = None
        if pull_directive_lines:
    ...
    # is derived straight from `upstream_ref` — a confirmed remote, or None
    # (including when the shared-history guard above just nulled it).
    ...
    if not boot_log_path:
        ...
        # file that was never written, silently losing the briefing content —
