...
def git_tracked_status(project_root: str, relpaths: list[str]) -> str:
    """Tri-state git-tracked check for `relpaths` (repo-relative) under
    `project_root` -- used by lib/dod_gate_classify.py to tell a module
    that was genuinely deleted (existed, got committed, got removed from
    the worktree without `git rm`) from one that was simply never
    written, WITHOUT collapsing "confirmed not tracked" and "git could
    not answer" into the same value.

    [2026-08-20, T1 fix, Cerberus/Argus Verify pass on stop-dod-gate.py]:
    the previous boolean `is_tracked_in_head()` returned False both when
    git genuinely said "not tracked" and when git itself failed to run
    (repo mid-operation, `.git` unreadable/renamed, git not on PATH,
    timeout). A caller treating that False as "safe to allow" -- exactly
    what `_classify_missing_module()` did -- would silently let a session
    close over a module that actually WAS deleted-but-tracked, just
    because git happened to be unavailable at the moment of the check.
    Reproduced directly: commit `moria/foo.py`, delete it from the
    worktree, rename `.git` away, invoke the classifier -- the boolean
    version returned False (looks identical to "never tracked") and the
    caller allowed the close.

    A bare pathspec that names a directory (e.g. "moria") matches every
    path git has indexed under it, not just an exact file named "moria" --
    this is plain `git ls-files -- <pathspec>` semantics, so passing a
    directory-shaped candidate alongside file-shaped ones in the same call
    is intentional, not a bug.

    Returns:
      "tracked"   -- `git ls-files` succeeded (exit 0) and at least one of
                     `relpaths` is indexed.
      "untracked" -- EITHER `git ls-files` succeeded (exit 0) and NONE of
                     `relpaths` is indexed, OR `project_root` is not
                     inside a git work tree at all (never initialized --
                     see the "not a real repo" note below). Both are
                     CONFIRMED "no tracked history exists here" answers a
                     caller may safely treat as "definitely absent".
      "unknown"   -- `project_root` IS inside a git work tree (so a real
                     repo with real history exists) but the actual
                     `ls-files` query still failed for some OTHER reason
                     (corrupted index, permission denied on `.git/`,
                     git binary crashing mid-operation, a real disk
                     error). Callers MUST treat this the same as
                     "tracked" for any decision that matters (D2: block
                     on doubt, never fail-open on a git-level unknown).
      `relpaths` empty also returns "unknown" (nothing to check, and
      "nothing to check" must not silently resolve to "confirmed
      absent" for a caller that skips the empty-list case).

    "Not a git work tree at all" is deliberately folded into "untracked",
    not "unknown" [2026-08-20, T1 hardening round 2, Cerberus/Argus Verify
    pass]: a project that has simply never run `git init` cannot have
    "genuinely deleted tracked code" -- there is no history to lose. The
    pre-check (`rev-parse --is-inside-work-tree`) below is what makes this
    safe rather than a repeat of the original T1 bug: it is ONLY reached
    when `ls-files` itself would also need a real repo to succeed, so a
    caller inside an ACTUAL repo whose `ls-files` fails for any other
    reason still lands on "unknown" and blocks, exactly as the T1 fix
    requires.

    Known, accepted limitation (verified by hand, not assumed): a
    directory whose `.git` was renamed/removed AFTER real commits existed
    is byte-for-byte indistinguishable, via any git command, from a
    directory that was NEVER a git repository -- both produce the
    identical `fatal: not a git repository (or any of the parent
    directories): .git`, exit 128. There is no git-observable signal left
    behind that separates "history existed and vanished" from "history
    never existed" once `.git` itself is gone; this function resolves
    that specific, contrived case as "untracked" (matching "never was a
    repo"), not "unknown". What this function DOES catch is the far more
    realistic failure this project's threat model cares about: a REAL,
    currently-present repo where `ls-files` itself errors out (corrupted
    index, permission issue, disk failure) while classifying a module
    that looks locally real -- that case still blocks.
    """
    if not relpaths:
        return "unknown"
    repo_code, _ = run_git(["rev-parse", "--is-inside-work-tree"], cwd=project_root)
    if repo_code != 0:
        return "untracked"
    code, output = run_git(["ls-files", "--", *relpaths], cwd=project_root)
    if code != 0:
        return "unknown"
    return "tracked" if output.strip() else "untracked"
