...
Fachada/api: `add()` (mas abajo) es la operacion real de esta pieza; el
resto se partio en tres hermanos por concernencia:

  - `rules_validate.py` -- valida `kind`/`text`/`quote` ANTES de tocar
    git o el fichero.
  - `rules_similarity.py` -- reconoce una linea ya escrita y detecta
    casi-duplicados por texto.
...
from rules_similarity import iter_rule_texts, similar_existing, strip_quote_suffix  # noqa: F401
...
def add(text: str, kind: str, quote=QUOTE_NOT_GIVEN) -> WriteResult:
    ...
    **Frontera aceptada, no se reabre sin que el propietario lo pida:**
    ...
    acepta porque `add()` es el UNICO escritor real de `rules.md` en todo
    el sistema (siempre bajo `lock_resource()`) -- solo una edicion
    manual que ignore la cabecera del fichero ("no editar") podria caer
