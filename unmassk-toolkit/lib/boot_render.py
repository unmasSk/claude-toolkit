...
Owns every `render_*_section()` function plus their small helpers (skill
drift/version checks, branch keyword parsing, time formatting, GitHub issue
matching, timeline extraction, relevance partitioning). Moved out of
hooks/session-start-boot.py verbatim (Cerberus T2-1): the hook file had grown
to 1110 lines, well past the project's 500-line limit, and these renderers
are cohesive as a unit — pure "given inputs, produce these briefing lines"
functions — distinct from main()'s orchestration, write_boot_log()'s file I/O,
render_boot_banner_lines()'s short-banner formatting, and
run_preboot_migrations()'s one-shot migration concerns, which all stay in the
hook file.

Pure refactor: behavior is byte-for-byte identical to before the split. See
lib/boot_memory.py's own module docstring and tests/test_migrate_statusline.py
for why `parsing` imports below are deferred into function bodies rather than
hoisted to module level — this module is a real, stably-named module (first
`import boot_render` anywhere in a process caches it for that process), and a
module-level `from parsing import X` could freeze X to a test's temporary
stub forever if this module's first-ever import happened to land inside that
stub's window.
"""
